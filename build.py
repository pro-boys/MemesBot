...
def method_fields(fields, method_name):
    ...
    for field in fields:
        ...
        field_type_text = ''
        # print(field.get('types'))
        """
        for types in field.get('types'):
            field_type, extra = get_field_type(types)
            print(field_type)
            #if "InputFile" in field_type:
            #    continue
            if field_name == "chatId" and field_type == "string":
                continue

            if SUBCLASS_DICT.get(field_type):
                field_type_text += extra + SUBCLASS_DICT.get(field_type)
                type_list.append(field_type_text)
                break
            else:
...
def construct_params(params, optionals, is_bool):
    ...
    for p in params:
        ...
        res += format_params(new_name, p['type'], new_p, True, is_bool)  # f'    params["{new_p}"] = {field_name[0].lower()}{field_name[1:]}\n'
    if len(optionals) > 0:
        res += '    if opts != nil {\n'
        for o in optionals:
            new_o = o['field']
            field_name = camel(new_o)
            res += format_params(field_name, o['type'], new_o, False, is_bool)  # f'        params["{new_o}"] = opts.{field_name}\n'
        res += '    }\n'
    return res
...
if __name__ == '__main__':
    type_content = build_types()
    type_content += get_unmarshals()
    write_types(type_content)
    method_content = build_methods()
